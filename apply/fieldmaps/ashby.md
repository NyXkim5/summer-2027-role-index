...
| `requires_sponsorship` | Do you or will you require sponsorship? | Yes/No button pair or radio | click-dropdown |
...
Ashby dropdowns are custom listboxes, not native selects. Click to
open, then click the option. Keyboard arrows plus Enter also work.

EEO handling: the section is voluntary self-identification and each
question has a decline option, usually "Decline to self identify". If
...
the matched library entry. Many orgs enable Autofill from resume,
which parses the upload and fills name, email, links, and education.
Upload first, wait for the filename to render in the widget and the
autofill pass to settle, then fill and verify every field in the
table, correcting whatever the parser guessed wrong.
...
factual screeners may match a key in `profile.custom` by label.
...
the park notes and write the log line.
